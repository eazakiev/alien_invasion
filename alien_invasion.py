...
class AlienInvasion:
    ...
    def _check_play_button(self, mouse_pos):
        ...
        if button_clicked and not self.stats.game_active:
            ...
            self.sb.prep_score()
            self.aliens.empty()  # Очистка списка пришельцев и снарядов
            self.bullets.empty()
            self._create_fleet()  # Создание нового флота и размещение корабля в центре
    ...
    def _update_screen(self):
        ...
        self.sb.show_score()  # Вывод инф-ии о счете
        if not self.stats.game_active:  # Кнопка Play отображается в том случае, если игра не активна
